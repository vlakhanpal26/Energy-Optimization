...
Part B
------
I continued this project with Part B. The new files for Part B are in the `partB/` folder:

- `partB/do_partbcode.py` — Python script implementing the Part B extension/analysis
- `partB/DO_PartB-2.ipynb` — Jupyter notebook with Part B exploration and results
- `partB/DO_PARTBReport.pdf` — Part B report
- `partB/Electrolyser_Use.csv` — additional dataset used in Part B

What I did in Part B
- Implemented an electrolyser usage analysis and extended the optimization code to incorporate electrolyser constraints and outputs.
- Added a notebook with exploratory analysis and a short report summarizing the Part B results.

How to run Part B
1. With the same virtual environment used for Part A (see Quick start above), open `partB/DO_PartB-2.ipynb` and run the notebook cells.
2. Or run the script directly:

	python partB/do_partbcode.py

Notes
- Part B is an extension of Part A and relies on some of the same input CSVs (electricity and hydrogen demand/prices). Keep both parts in the same repository to make cross-referencing and comparison easy.
